<template>
  <div class="youquan clearfix">
        ...
        <div class="youquan_a" :key="index" v-for="(item, index) in informations">
          ...
          <div class="youquan_a_bottom">
            <div class="control">
              ...
              <div @click="change_open(item.s_id,item.imgs,item.content)">
          ...
          <div
            ...
            @click.stop="change_opens(item.s_id,item.imgs,item.content,items.c_username,items.nickName)"
          >
    ...
    <loading :show="hasMore" text></loading>
    <div v-transfer-dom v-if="imageUrlok">
      <previewer :list="imageUrl" ref="previewers" @on-close="closes" :options="options"></previewer>
    </div>
  </div>
</template>
...
pullUpLoad: {
  threshold: 100,
      ...
      sendinformation: {
        ...
        recontact: null,
        time: "",
        username_zy: store.state.user.username_zy,
  ...
  computed: {
    names(){
      return  store.state.user.username_zy==store.state.user.username ?  "我的动态" : "Ta的动态";
    },
    imageUrlok() {
    ...
    xinxiy() {
      return store.state.user.konjian[store.state.user.username_zy];
    }
  },
  created() {
    this.initData();
  ...
  methods: {
      ...
      if (result < 0) {
        this.information.forEach(item => {
          if (item.s_id == s_id) {
            ...
            this.sendinformations.username_zy=username
            item.likes = item.likes+1;
    ...
    websocketsends() {
      ...
      store.state.user.websock.send(
        JSON.stringify(createComment(this.sendinformation))
      );
      this.$nextTick(() => {
    ...
    change_opens(sid,imgs,content,username,nickNames) {
  ...
  watch: {
    xinxiy: {
      handler(newValue, oldValue) {
        this.information.some((item, i) => {
          ...
          // if (item.s_id == newValue[newValue.length - 1].S_Id&&(newValue[newValue.length - 1].type!="comments_send")) {
    ...
    $route(to, from) {                                                       
      if (from.meta.keepAlive == false && from.path == "/publishpage"&& to.path==`/zhuye/${store.state.user.username_zy}`) {
        this.opensdf=false;
        this.scrollToTop();
